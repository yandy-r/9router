...
export function kiroToClaudeResponse(chunk, state) {
  ...
  if (data.usage && typeof data.usage === "object") {
    ...
    const cacheRead =
      data.usage.cache_read_input_tokens ?? data.usage.prompt_tokens_details?.cached_tokens;
    const cacheCreation =
  ...
  if (!state.messageStartSent) {
    ...
    state.messageId =
      (typeof data.id === "string" && data.id.replace("chatcmpl-", "")) || `msg_${Date.now()}`;
...
export function kiroToClaudeNonStreaming(data) {
  ...
  return {
    ...
    usage: {
      ...
      ...(typeof (usage.cache_read_input_tokens ?? usage.prompt_tokens_details?.cached_tokens) ===
      "number"
        ? {
