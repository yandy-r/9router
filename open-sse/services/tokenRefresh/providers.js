...
function buildRefreshBody(profile, config, refreshToken) {
  ...
  const includeSecret =
    profile?.includeClientSecret === undefined
      ? true
...
export async function refreshGoogleToken(refreshToken, clientId, clientSecret, log) {
  ...
  if (!clientId || !clientSecret) {
    log?.error?.(
      "TOKEN_REFRESH",
      "Google OAuth client not configured: set GEMINI_OAUTH_CLIENT_ID/_SECRET (gemini, gemini-cli) or ANTIGRAVITY_OAUTH_CLIENT_ID/_SECRET (antigravity)",
    );
    return null;
  }
  return dedupRefresh(
...
export async function refreshWindsurfToken(credentials, log) {
  log?.info?.("TOKEN_REFRESH", "windsurf: apiKey is long-lived (no refresh_token flow) — skipping");
  return null;
}
