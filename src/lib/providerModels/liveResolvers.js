...
async function fetchAllAnthropicModels(authHeaders) {
  const headers = { "Anthropic-Version": ANTHROPIC_API_VERSION, "Content-Type": "application/json", ...authHeaders };
  const data = [];
  let afterId = null;
...
const parseAnthropicModels = (body) => (body?.data || [])
  .filter((m) => m?.id)
...
const resolveClaudeOAuthModels = buildOAuthResolver({
  ...
  fetchFn: (token) => fetchAllAnthropicModels({ "Authorization": `Bearer ${token}`, "Anthropic-Beta": ANTHROPIC_OAUTH_BETA }),
...
async function resolveClaude(connection) {
  if (connection.accessToken) {
    ...
    return result.models.length ? result : { models: [], warning: result.warning || noModels("Claude").warning };
  }
  ...
  if (!response.ok) {
    return { models: [], warning: `Failed to fetch Claude models: ${response.status} ${await response.text()}` };
  }
...
async function resolveZed(connection, { forceRefresh }) {
  ...
  return { models: [], warning: await explainEmptyZedCatalog(credentials, result, { config: ZED_HOSTED_CONFIG }) };
}
...
async function resolveGrokCli(connection) {
  ...
  const result = await resolveGrokCliModels({ ...connection, connectionId: connection.id }, {
    log: console,
    proxyOptions: {
...
export const LIVE_MODEL_RESOLVERS = {
  ...
  cursor: passthrough("Cursor", (conn, { forceRefresh }) => resolveCursorModels({
    accessToken: conn.accessToken,
    providerSpecificData: conn.providerSpecificData || {},
  }, { forceRefresh, log: console })),
  kimchi: passthrough("Kimchi", (conn, { forceRefresh }) => resolveKimchiModels({
    accessToken: conn.accessToken,
    apiKey: conn.apiKey,
    providerSpecificData: conn.providerSpecificData || {},
  }, { forceRefresh, log: console })),
  github: passthrough("GitHub Copilot", (conn, { forceRefresh }) => resolveCopilotModels({
    accessToken: conn.accessToken,
  ...
  cline: passthrough("Cline", (conn) => resolveClineModels({ accessToken: conn.accessToken, apiKey: conn.apiKey })),
  clinepass: passthrough("ClinePass", (conn) => resolveClinepassModels({ accessToken: conn.accessToken, apiKey: conn.apiKey })),
};
...
export async function resolveLiveModels(connection, { forceRefresh = false } = {}) {
  ...
  if (!resolver) return { models: [], warning: `No live model catalog for ${connection?.provider}` };
  ...
  try {
    ...
    result = { models: [], warning: `Failed to fetch ${connection.provider} models: ${error?.message || error}` };
  }
  if (result.models.length > 0) cache.set(key, { result, expiresAt: Date.now() + LIVE_MODELS_TTL_MS });
