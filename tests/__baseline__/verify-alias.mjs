...
const sortedIdToAlias = Object.fromEntries(
  Object.keys(PROVIDER_ID_TO_ALIAS)
    .sort()
    .map((k) => [k, PROVIDER_ID_TO_ALIAS[k]]),
);
const resolved = {
...
if (!existsSync(snapPath)) {
  console.error("No baseline. Run --snapshot first.");
  process.exit(1);
}
...
if (JSON.stringify(baseline.idToAlias) !== JSON.stringify(current.idToAlias))
  console.error("~ idToAlias changed");
if (JSON.stringify(baseline.modelKeys) !== JSON.stringify(current.modelKeys))
  console.error("~ modelKeys changed");
process.exit(1);
