...
describe("prefetchRemoteImages", () => {
  ...
  it("openai source -> ollama target: converts remote URL to base64", async () => {
    ...
    expect(n).toBe(1);
    expect(body.messages[0].content[0].image_url.url.startsWith("data:image/png;base64,")).toBe(
      true,
    );
  });
  ...
  it("gemini source -> gemini target: fileData URL -> inlineData base64", async () => {
    const body = {
      contents: [
        {
          role: "user",
          parts: [{ fileData: { mimeType: "image/png", fileUri: "https://x/a.png" } }],
        },
      ],
    };
