...
import { buildAgentRunFrame, classifyCursorError, CursorExecutor } from "../../open-sse/executors/cursor.js";
...
describe("SelectedContext.selected_images encoder", () => {
  it("encodes uuid(2), dimension(4), mime_type(7), raw data(8)", () => {
    const msg = decodeMessage(encodeSelectedContextImages([
...
describe("image turn → AgentService frame", () => {
  it("attaches a current-turn image as raw bytes with parsed dimensions", async () => {
    const messages = [{ role: "user", content: [{ type: "text", text: "what is this?" }, imagePart()] }];
  ...
  it.each([
    ...
    await expect(resolveCursorImages([{ role: "user", content: [imagePart(url)] }])).rejects.toThrow(CursorImageError);
  });
...
describe("Cursor Update Required is account-neutral", () => {
  ...
  it("maps to 400 and never locks or rotates the account", () => {
    ...
    expect(checkFallbackError(detail.status, detail.message)).toEqual({ shouldFallback: false, cooldownMs: 0 });
  });

  it("keeps genuine resource_exhausted on 429 backoff", () => {
    const detail = classifyCursorError({ error: { code: "resource_exhausted", message: "slow down" } });
