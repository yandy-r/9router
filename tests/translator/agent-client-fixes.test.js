...
describe("Kiro tool names round-trip", () => {
  ...
  it("omits the map when no name changed", () => {
    const body = screenshotTurn();
    body.tools = [
      { name: "plain_tool", description: "x", input_schema: { type: "object", properties: {} } },
    ];
  ...
  it("restores the client name on streamed Claude tool_use blocks", () => {
    const state = {
      ...
      toolNameMap: new Map([["mcp_browser_computer", "mcp__browser__computer"]]),
    };
    const chunk = {
      id: "c1",
            ...
            tool_calls: [
              {
                ...
                id: "call_1",
                type: "function",
                function: { name: "mcp_browser_computer", arguments: "" },
              },
            ],
          },
          finish_reason: null,
...
describe("redact-thinking beta follows the client's display request", () => {
  it("keeps redact-thinking by default and drops it for summarized display", () => {
    ...
    expect(
      selectAnthropicBeta("claude-sonnet-5", {
        thinking: { type: "adaptive", display: "omitted" },
      }),
    ).toContain("redact-thinking-2026-02-12");
    const summarized = selectAnthropicBeta("claude-sonnet-5", {
      thinking: { type: "adaptive", display: "summarized" },
    });
