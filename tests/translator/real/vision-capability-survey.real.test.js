...
const ACTIVE_PROBES = (process.env.PROBES || "vision")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const CRED_MSG_RE =
  /subscription|unauthorized|invalid api key|invalid access token|insufficient|credits|payment|spending|organization policy|disallowed|quota|exhausted|not supported when using|not available for integrator|requires a subscription|model.*not found|does not exist|not yet known|requires a role/i;
...
function chatModels(providerId) {
  return getModelsByProviderId(providerId).filter(
    (m) => !NON_CHAT_KINDS.has(m.kind || m.type || "llm"),
  );
}
...
describe.skipIf(!RUN_REAL)("REAL vision capability survey", () => {
        ...
        it.concurrent(
          ...
          async () => {
            ...
            if (credIssue) verdict = "skip-cred";
              ...
              verdict = `ok-no${probeName}-tolerated`; // provider silently ignored modality
            else if (!ok && !capOn)
              verdict = `FAIL-no${probeName}-needs-strip`; // target problem
            ...
            results.push({
              ...
              status,
              verdict,
              error: ok ? "" : String(result.error || "").slice(0, 80),
            });
  ...
  afterAll(() => {
    ...
    const groups = [...new Set(results.map((r) => r.verdict))].sort(
    ...
    for (const g of groups) {
      ...
      for (const r of rows) {
        console.log(
          `  [${r.status}] ${r.probe} ${r.providerId}/${r.model} cap=${r.cap}${r.error ? ` :: ${r.error}` : ""}`,
        );
      }
    }
    console.log("\n================ END SURVEY ================\n");
