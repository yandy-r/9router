...
describe.skipIf(!RUN_REAL)("REAL provider behavior cases", () => {
  ...
  it(
    ...
    async () => {
      ...
      if (!out) return expect(true).toBe(true);
      expect(
        /"finish_reason"\s*:\s*"tool_calls"/.test(out.raw),
        "no tool_calls finish_reason",
      ).toBe(true);
  ...
  it(
    "codex: session stickiness (cached_tokens on 2nd turn)",
    async () => {
