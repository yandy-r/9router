...
function buildFileBody(fmt, mime, b64) {
  ...
  if (fmt === "gemini" || fmt === "gemini-cli" || fmt === "antigravity") {
    const gem = {
      contents: [
        { role: "user", parts: [{ text: ask }, { inlineData: { mimeType: mime, data: b64 } }] },
      ],
      ...
      messages: [
        {
          ...
          content: [
            { type: "text", text: ask },
            {
              type: "file",
              file: {
                filename: `doc.${mime === PDF_MIME ? "pdf" : "docx"}`,
...
const FILE_TYPES = [
  ["pdf", PDF_MIME, PDF_B64],
  ["docx", DOCX_MIME, DOCX_B64],
];
...
describe.skipIf(!RUN_REAL)("REAL file base64 survey", () => {
  ...
  afterAll(() => {
    ...
    if (!results.length) {
      w("[file-survey] no results collected");
      return;
    }
    ...
    for (const g of groups) {
      ...
      for (const r of rows) {
        w(
          `  [${r.status}] ${r.kind} ${r.providerId}/${r.model} fmt=${r.fmt}${r.error ? ` :: ${r.error}` : ""}`,
        );
      }
    }
    w("\n================ END SURVEY ================\n");
